...
- **一次登录只存活于发起它的进程中**：授权尝试不可持久，在登录途中关闭发起它的界面会丢弃它，人需要重来；Web 模型页已经可以经[授权的 Web 线路](../../credentials/authorization/README.zh.md)发起这些登录，同样适用该规则。登出即对已存储记录执行 `deleteRecord`，它只在本地遗忘而不通知签发方。
