...
- **A sign-in lives only in the process that started it** — an authorization attempt is not durable, so closing the initiating surface mid-login abandons it and the human starts over; the Web Models page starts these sign-ins through [the authorization web wire](../../credentials/authorization/README.md), under the same rule. Signing out is `deleteRecord` on the stored record, which forgets it locally without telling the issuer.
